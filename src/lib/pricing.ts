...
export function computePricing(input: PricingInput): PricingBreakdown {
  ...
  const installPerCabinet = num(s.installation_fee_per_cabinet, 12);
  const dismantlePerCabinet = num(s.dismantling_fee_per_cabinet, 8);
  const transportBase = num(s.transport_fee_base, 600);
  const transportPerCabinet = num(s.transport_fee_per_cabinet, 3);
  const technicianDailyRate = num(s.technician_daily_rate, 600);
  const processorDailyRate = num(s.processor_daily_rate, 450);
  ...
  const minimumRentalPrice = num(s.minimum_rental_price, 2500);
  ...
  const technicianFee = input.includeTechnician ? technicianDailyRate * days : 0;
