...
import { deleteObject, inspectObject } from "@/lib/storage/s3";
import { MAX_PRIVATE_MEDIA_BYTES, MAX_PUBLIC_MEDIA_BYTES, validatedImageDimensions, validateMediaBytes } from "@/lib/storage/validation";
import { writeAudit, requestAuditMetadata } from "@/lib/admin/audit";

export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  let mediaId = 0;
  let pendingObject: { bucket: string; objectKey: string } | null = null;
  try {
    ...
    pendingObject = { bucket: asset.bucket, objectKey: asset.objectKey };
    ...
    const dimensions = validatedImageDimensions(inspected.bytes, asset.mimeType);
    ...
    if (pendingObject) await deleteObject(pendingObject.bucket, pendingObject.objectKey).catch(() => undefined);
